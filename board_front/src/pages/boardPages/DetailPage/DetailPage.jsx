...
function DetailPage(props) {
    ...
    const [ commentData, setCommentData ] = useState({
        boardId, // 키값이랑 변수명이 같으면 생략가능 ex) boardId = boardId
        parentId: null,
        content: "",
    });
    ...
    const commentMutation = useMutation(    // useMutation: 작성, 수정, 삭제할때 사용
        async () => {
    ...
    const modifyCommentMutation = useMutation(
        async () => await instance.put(`/board/comments/${commentModifyData.commentId}`, commentModifyData),
        {
            onSuccess: response => {
